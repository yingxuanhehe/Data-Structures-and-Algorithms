...
void reverseFirstKItems(Queue *q, int k)
{
    ...
    {
        item = dequeue(q);
        push(&s, item);     //store the k numbers into stack
    }

    for(i=0; i<(totalSize-k); i++)
    {
        item = dequeue(q);
        enqueue(&temp, item); //store the rest of the numbers in to another queue
    }

    for(i=0; i<totalSize; i++)
    {
        if(i<k)
            enqueue(q, pop(&s));         //store the reversed k items back to q

        else
            enqueue(q, dequeue(&temp));    //after storing them, store the rest of the items(unreversed) from qq back to q
    }
