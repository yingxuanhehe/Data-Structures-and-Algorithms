...
void levelOrderTraversal(BSTNode* root)
{
    ...
    enqueue(&(q.head), &(q.tail), root); //enqueue the root

    while(!isEmpty(q.head))              //while q is not empty
    {
        BSTNode *temp = dequeue(&(q.head), &(q.tail));  //dequeue one item from q and print it
        printf("%d", temp->item);

        if(temp->left != NULL)                          //enqueue the item's children
