...
int removeNode(ListNode **ptrHead, int index){
	...
	if(*ptrHead == NULL)  //means head==NULL
        return -1;
...
int duplicateReverse(ListNode *head, ListNode **ptrNewHead)
{
    ...
    //traverse the list and insert each visited node into the new list at index 0 every time
    while(cur!=NULL)
    {
...
int moveMaxToFront(ListNode **ptrHead){
    ...
    //If preMax is still NULL at the end, first node has the max value
    ...
    //Move node with max value to the first position
    cur = preMax->next;
...
int concatenate(ListNode **ptrHead1, ListNode *head2)
{
	...
	//Both lists empty, return -1 error value;
    ...
    //Empty second list, do nothing
    if(head2 == NULL)
        return 0;

    //Traverse the first list to get the last node
    //We check for the value of the next pointer for the current node
...
int combineAlternating(ListNode **ptrHead, ListNode *head1, ListNode *head2){
    ...
    {
        //Adding the first node, remember to update the head pointer

        if(*ptrHead == NULL)
        {
            if(head1 != NULL)
            {
                insertNode(ptrHead, 0, head1->num);
                //same as:
                //*ptrHead = malloc(sizeof(ListNode));
                //(*ptrHead)->num = head1->num;

                printf("%d", (*ptrHead)->num);
                temp = *ptrHead;
                head1 = head1->next;

                if(head2 != NULL)
                {
                    insertNode(ptrHead, 1, head2->num);
                   // same as:
                   // temp->next = malloc(sizeof(ListNode)); //why cnt use (*ptrHead)->next
